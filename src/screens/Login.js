...
const Login = ({ navigation }) => {
  ...
  const handleLogin = () => {
    if (email && password) {
      if (!validateEmail(email)) {
        ...
        // Perform login logic, e.g., send data to an API, authenticate user, etc.
        ...
        navigation.navigate('MyRoom'); // for HOME!!!!!!!!!!!!!
      }
    } else {
  ...
  return (
    <View style={styles.container}>
      <Image source={require('../assets/images/Loginpic.gif')} style={styles.LogInPic} />
      ...
      <TextInput
        ...
        <TextInput
          ...
          onFocus={() => setInputErrors((prevErrors) => ({ ...prevErrors, password: false }))}
        />
        <TouchableOpacity
        
          style={[styles.button, { backgroundColor: colors.main.button }]}
          onPress={handleLogin}
        >
          <Text style={styles.buttonText}>Login</Text>
        </TouchableOpacity>
        {loginupStatus !== '' && <Text style={styles.loginStatusText}>{loginupStatus}</Text>}
