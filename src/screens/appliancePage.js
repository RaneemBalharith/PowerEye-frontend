...
export default function AppliancePage({ navigation }) {
  const [applianceName, setApplianceName] = useState('Appliance'); // Set an initial room name
  ...
  const handleUpdateApplianceName = () => {
    if (updatedApplianceName.length < 2) {
      ...
      setApplianceName(updatedApplianceName);
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={{ paddingTop: 30, paddingLeft: 30, fontSize: 18, marginRight: 5 }}>
        Current Power:
        <Text style={{ fontSize: 20, color: '#00707C', fontWeight: 'bold', alignItems: 'center' }}>5</Text>
      </Text>
