...
import { addApplianceRequest } from "../../api/apiManager";
const windowWidth = Dimensions.get("window").width;
const windowHeight = Dimensions.get("window").height;

export const AddDeviceModal = ({ onAddDevice, setOnAddDevice }) => {
  const {smartPlugs,token,setRefresh} = useContext(PowerEyeContext)
  ...
  const onAddNewDevice = () =>{
    addApplianceRequest(token,deviceName,selectedPlug,selectedDevice).then((res)=>{
      setRefresh(res)
    })
    setSelectedPlug()
    setSelectedDevice()
